...
function isPalindrome(str) {
  const preparatedStr = str
    .toLowerCase()
    .replace(/,/g, '')
    .replace(/ /g, '')
    .replace(/\?/g, '')
    .replace(/!/g, '');
  const reversedStr = preparatedStr.split('').reverse().join('');
  if (preparatedStr === reversedStr) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  const reversWords = [];
  words.forEach((item) => reversWords.push(item.split('').reverse().join('')));
...
function encodeToRot13(str) {
  ...
  function convert(char) {
    let convertedChar;
    if (firstArr.includes(char.toLowerCase())) {
      convertedChar = secondArr[firstArr.indexOf(char.toLowerCase())];
    } else if (secondArr.includes(char.toLowerCase())) {
      convertedChar = firstArr[secondArr.indexOf(char.toLowerCase())];
    } else {
      convertedChar = char;
    }
